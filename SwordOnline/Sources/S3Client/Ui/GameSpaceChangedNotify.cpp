...
pFile->
WriteString(lpSection, szKey, szLine
);
i++;
iU++;
}
...
int
BlacklistNotify::RenameUnitGroup(const char *Unit, const char *Name, const char *NewName, const STRINGLIST &friends) {
...
void KClientCallback::ChannelMessageArrival(DWORD nChannelID, char *szSendName, const char *pMsgBuff,
                                            unsigned short nMsgLength, const char *pItem, BYTE btSomeFlag, bool bSucc) {
        ...
        if (g_pCoreShell->FindSpecialNPC(szSendName, &SelectPlayer, nKind) && nKind == kind_player) {
            if (pItem) {
                ...
                int nIdx = g_pCoreShell->GetGameData(GDI_ITEM_CHAT, true,
                                                     (int) &m_pItem); //TamLTM note Post item kenh chat
                if (nIdx) {
...
void KClientCallback::MSNMessageArrival(char *szSourceName, char *szSendName, const char *pMsgBuff,
                                        unsigned short nMsgLength, const char *pItem, bool bSucc) {
    if (nMsgLength >= MAX_MESSAGE_LENGTH)
        return;

    if (!bSucc) {
        char szInfo[256];
        int n = sprintf(szInfo, "Xin th«ng c¶m, %s kh«ng ë trong Vâ L©m TruyÒn Kú, tin tøc kh«ng thÓ göi ®Õn!",
                        szSendName);
        KUiMsgCentrePad::SystemMessageArrival(szInfo, n);
        return;
    }
