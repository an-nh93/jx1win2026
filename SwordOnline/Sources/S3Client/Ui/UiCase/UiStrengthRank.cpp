    ...
    if (Ini.Load(INDEX_AND_RANK_MAPPING)) {
        ...
        if (pIndex) {
            ...
            for (i = 0; i < nCount; i++) {
                ...
                pIndex[i].usIndexId = (unsigned short) j;
            ...
            if (m_pCurrentIndexNode->bValueAppened) {
                ...
                if (m_pCurrentIndexNode->szUnit[0]) {
                    if (m_pCurrentIndexNode->usIndexId == enumTopTenMasterHand ||
                        (m_pCurrentIndexNode->usIndexId >= enumFacTopTenMasterHand &&
                         m_pCurrentIndexNode->usIndexId < enumFacTopTenRicher)
                            )
                        sprintf(szBuf, "C蕄 %d", pNode->nValueAppend % MAX_LEVEL +
                                                 (pNode->nValueAppend % MAX_LEVEL == 0 ? MAX_LEVEL : 0),
                                pNode->nValueAppend / MAX_LEVEL - (pNode->nValueAppend % MAX_LEVEL == 0 ? 1 : 0));
