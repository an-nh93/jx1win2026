...
void KReconnectWnd::Breathe() {
    ...
    char szInfo[256];
    int nInfoLen;
    if (m_bWaitToReconnect) {    //====µÈ´ý½øÐÐÏÂ´Î×Ô¶¯ÖØÁ¬====
    ...
    switch (eStatus) {
        ...
        case LL_S_IDLE:    //====ÖØÁ¬Ê§°Ü====
        {
            ...
            if (eResult == LL_R_ACCOUNT_NOT_ENOUGH_POINT) {
                ...
                m_bStop = true;
                pReconnectMsg = "Tµi kho¶n nµy ®· bÞ khãa";
            } else {
                m_bWaitToReconnect = true;
                m_uWaitStartTime = IR_GetCurrentTime();
                if (m_nHideTimes > 0) {
            ...
            break;
        case LL_S_ACCOUNT_CONFIRMING:    //µÈ´ýÕËºÅÃÜÂëÑéÖ¤
            pReconnectMsg = "§ang tiÕn hµnh kiÓm tra tµi kho¶n vµ mËt khÈu";
            break;
        case LL_S_WAIT_ROLE_LIST:        //µÈ´ý½ÓÊÕ½ÇÉ«ÁÐ±íÊý¾Ý
            pReconnectMsg = "§ang nhËn danh s¸ch nh©n vËt";
            break;
        case LL_S_WAIT_TO_LOGIN_GAMESERVER:    //µÈ´ýµÇÂ½ÓÎÏ··þÎñÆ÷
            pReconnectMsg = "§ang phôc vô trß ch¬i";
            break;
        case LL_S_ENTERING_GAME:        //ÕýÔÚ½øÈëÓÎÏ·
...
int KReconnectWnd::WndProc(unsigned int uMsg, unsigned int uParam, int nParam)//´°¿Úº¯Êý
{
