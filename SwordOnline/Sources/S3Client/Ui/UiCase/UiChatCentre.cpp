    ...
    switch (uMsg) {
        ...
        case WND_M_MENUITEM_SELECTED:
            if (uParam == (unsigned int) (KWndWindow *) this) {
                ...
                } else if (HIWORD(nParam) >= SEL_GROUP_MENU_SELECT_BASE && (short) (LOWORD(nParam)) >= 0) {
            ...
            if (nParam) {
                if (uParam == ISP_NEW_GROUP) {
                    ...
                    if (nGroupIndex >= 0 && nGroupIndex < MAX_GROUP_NUM) {
                        ReplaceGroupName(m_nCurentFriendUnit,
                                         m_AllFriendUnits[m_nCurentFriendUnit].m_GroupInfo[nGroupIndex].strGroupName.c_str(),
                                         (const char *) nParam);
                    }
                }
            }
...
void KUiChatCentre::OnMoveFriendToGroup(int nFriendIndex, int nIndexInMenu) {
    if (nFriendIndex >= 0 && nFriendIndex < m_nNumFriendInList) {
        for (int nNewGroupIndex = 0;
             ...
             nNewGroupIndex++) {
            if (m_bValidGroup[nNewGroupIndex] &&
                nNewGroupIndex != m_AllFriendUnits[m_nCurentFriendUnit].m_nCurExpandGroup) {
                if ((nIndexInMenu--) == 0) {
                    MoveFriendToGroup(m_nCurentFriendUnit, m_pFriendList[nFriendIndex].Name,
                                      m_AllFriendUnits[m_nCurentFriendUnit].m_GroupInfo[nNewGroupIndex].strGroupName.c_str());
                }
            }
        }
    }
...
void KUiChatCentre::OnSelGroupMenu(int nGroupIndex, int nCmd) {
    ...
    switch (nCmd) {
        ...
        case GROUP_MENU_ITEM_DELETE: {
            MoveGroupToGroup(m_nCurentFriendUnit,
                             m_AllFriendUnits[m_nCurentFriendUnit].m_GroupInfo[nGroupIndex].strGroupName.c_str(), "",
                             true);
        }
            break;
...
bool KUiChatCentre::AddFriendInfo(int nUnitIndex, char *Name, char *group) {
    ...
    if (m_pSelf) {
        ...
        if (m_pSelf->m_AllFriendUnits[nUnitIndex].m_FriendMap.find(Name) !=
            m_pSelf->m_AllFriendUnits[nUnitIndex].m_FriendMap.end()) {    //ÒÑ´æÔÚÓÚÁÐ±íÖÐ
        ...
        if (m_pSelf->m_AllFriendUnits[nUnitIndex].m_UnitName ==
            m_pSelf->m_AllFriendUnits[m_pSelf->m_nCurentFriendUnit].m_UnitName) {
            m_pSelf->UpdateAll(m_pSelf->m_AllFriendUnits[m_pSelf->m_nCurentFriendUnit].m_nCurExpandGroup);
            m_pSelf->UpdateInterface();
        }
        return true;
    ...
    if (m_pSelf) {
        ...
        for (int i = 1; i < MAX_GROUP_NUM; i++) {
            if (m_pSelf->m_AllFriendUnits[nUnitIndex].m_GroupInfo[i].strGroupName == strGroup) {
                ...
                if (m_pSelf->m_AllFriendUnits[nUnitIndex].m_pNotify)
                    m_pSelf->m_AllFriendUnits[nUnitIndex].m_pNotify->CreateUnitGroup(
                            m_pSelf->m_AllFriendUnits[nUnitIndex].m_UnitName.c_str(), strGroup.c_str());
        ...
        if (bDelgroup) {
            if (nSrcIndex != 0)    //0ÊÇÃ¿¸öUnitµÄÄ¬ÈÏ×é,²»ÄÜÉ¾³ý
            {
                for (; nSrcIndex < MAX_GROUP_NUM - 1; nSrcIndex++) {
                    m_pSelf->m_AllFriendUnits[nUnitIndex].m_GroupInfo[nSrcIndex] = m_pSelf->m_AllFriendUnits[nUnitIndex].m_GroupInfo[
                            nSrcIndex + 1];
                }
    ...
    if (m_pSelf) {
        for (int i = 0; i < m_pSelf->m_nFriendUnitCount; i++) {
            if (m_pSelf->m_AllFriendUnits[i].m_pNotify == &ms_SNotify &&
                m_pSelf->m_AllFriendUnits[i].m_FriendMap.find(roleName) !=
                m_pSelf->m_AllFriendUnits[i].m_FriendMap.end()) {
                return i;
            }
        }
    }
    ...
    if (m_pSelf) {
        ...
        if (iF != m_pSelf->m_AllFriendUnits[nUnitIndex].m_FriendMap.end()) {
            ...
            if (m_pSelf->m_AllFriendUnits[nUnitIndex].m_pNotify && bNotify)
                m_pSelf->m_AllFriendUnits[nUnitIndex].m_pNotify->DeleteUnitMember(
                        m_pSelf->m_AllFriendUnits[nUnitIndex].m_UnitName.c_str(), Name);
...
void KUiChatCentre::PopupSelGroupMenu(int nFriendIndex) {
    ...
    for (int i = 0; i < MAX_GROUP_NUM && m_pSelGrupMenu->nNumItem < m_nNumValidGroup - 1; i++) {
        if (m_bValidGroup[i] && i != m_AllFriendUnits[m_nCurentFriendUnit].m_nCurExpandGroup) {
            ...
            m_pSelGrupMenu->Items[m_pSelGrupMenu->nNumItem].uDataLen = strlen(
                    m_pSelGrupMenu->Items[m_pSelGrupMenu->nNumItem].szData);
        ...
        if (m_pFriendList) {
            ...
            for (; i != m_AllFriendUnits[m_nCurentFriendUnit].m_FriendMap.end(); i++) {
                if (strcmp(i->second.c_str(), groupname) == 0) {
                    if (m_AllFriendUnits[m_nCurentFriendUnit].m_FriendOnlineMap.find(i->first) !=
                        m_AllFriendUnits[m_nCurentFriendUnit].m_FriendOnlineMap.end()) {
...
void KUiChatCentre::UpdateInterface() {
    ...
    for (; nGroupIndex <= m_AllFriendUnits[m_nCurentFriendUnit].m_nCurExpandGroup &&
           nGroupIndex < MAX_GROUP_NUM; nGroupIndex++) {
        if (m_bValidGroup[nGroupIndex]) {
