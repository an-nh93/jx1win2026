            ...
            if (uParam == (unsigned int) (KWndWindow *) &m_Close /*&& !g_UiBase.GetStatus()*/) //Fix close status
            {
                ...
                if (uParam == (unsigned int) (KWndWindow *) &m_AddStrength) {
                    if (GetKeyState(VK_CONTROL) & 0x8000)
                        KUiGetString::OpenWindow(GSA_NORMAL, "NhËp Sè ®iÓm", "", this, UIITEM_WAIT_GETSTR,
                                                 m_nRemainPoint);
                    else
                        UseRemainPoint(0, 1);
                } else if (uParam == (unsigned int) (KWndWindow *) &m_AddDexterity) {
                    if (GetKeyState(VK_CONTROL) & 0x8000)
                        KUiGetString::OpenWindow(GSA_NORMAL, "NhËp Sè ®iÓm", "", this, UIITEM_WAIT_GETDEX,
                                                 m_nRemainPoint);
                    else
                        UseRemainPoint(1, 1);
                } else if (uParam == (unsigned int) (KWndWindow *) &m_AddVitality) {
                    if (GetKeyState(VK_CONTROL) & 0x8000)
                        KUiGetString::OpenWindow(GSA_NORMAL, "NhËp Sè ®iÓm", "", this, UIITEM_WAIT_GETVIT,
                                                 m_nRemainPoint);
                    else
                        UseRemainPoint(2, 1);
                } else if (uParam == (unsigned int) (KWndWindow *) &m_AddEnergy) {
                    if (GetKeyState(VK_CONTROL) & 0x8000)
                        KUiGetString::OpenWindow(GSA_NORMAL, "NhËp Sè ®iÓm", "", this, UIITEM_WAIT_GETENE,
                                                 m_nRemainPoint);
                    else
                        UseRemainPoint(3, 1);
                }
