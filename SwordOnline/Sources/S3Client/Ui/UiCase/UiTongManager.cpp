    ...
    switch (uMsg) {
        ...
        case WND_N_BUTTON_CLICK:
            if (uParam == (unsigned int) &m_BtnPrev) {
                ...
                KUiTongGetString::OpenWindow(true, "§æi danh hiÖu", NULL, this, ISP_NEW_AGNAME, 4,
                                             defTONG_NAME_LENGTH_12);
            else if (uParam == (unsigned int) &m_BtnChangeMaleTitle)
                KUiTongGetString::OpenWindow(true, "§æi hiÖu nam", NULL, this, ISP_NEW_MALEAGNAME, 4,
                                             defTONG_NAME_LENGTH_12);
            else if (uParam == (unsigned int) &m_BtnChangeFemaleTitle)
                KUiTongGetString::OpenWindow(true, "§æi hiÖu n÷", NULL, this, ISP_NEW_FEMALEAGNAME, 4,
                                             defTONG_NAME_LENGTH_12);
...
void KUiTongManager::UpdateTongInfoView() {
    ...
    int nMsgLen = sprintf(Buff,
                          "<color=189,190,189>LËp bang: <color=0,255,0>%02d:%02d:%02d %02d-%02d-%04d<color>\n\nCèng hiÕn tuÇn tr­íc: <color=0,255,0>%d<color>\nCèng hiÕn tuÇn nµy: <color=0,255,0>%d<color>\nCèng hiÕn kh¶ dông: <color=0,255,0>%d<color><color>",
                          timeinfo->tm_hour, timeinfo->tm_min, timeinfo->tm_sec, timeinfo->tm_mday,
                          timeinfo->tm_mon + 1, timeinfo->tm_year + 1900,
                          KSG_StringGetValue(3, m_TongData.nMasterSaveEff, 1),
                          KSG_StringGetValue(3, m_TongData.nMasterSaveEff, 2),
                          KSG_StringGetValue(3, m_TongData.nMasterSaveEff, 3));
    ...
    m_MemberCount.SetText(
            itoa(m_TongData.nMemberCount + m_TongData.nManagerCount + m_TongData.nDirectorCount, Buff, 10));
            ...
            if (pNode) {
                ...
                for (int i = nViewCount; i < nDataCount; i++) {
                    ...
                    if (m_List.AddString(i, szMsgBuff) > WNDLIST_ERROR) {
                        ...
                        struct tm *timeinfo = localtime(&rawtime);
                        nMsgLen = sprintf(szMsgBuff,
                                          "<color=189,190,189>NhËp bang: <color=0,255,0>%02d:%02d:%02d %02d-%02d-%04d<color>\n\nCèng hiÕn tuÇn tr­íc: <color=0,255,0>%d<color>\nCèng hiÕn tuÇn nµy: <color=0,255,0>%d<color>\nCèng hiÕn kh¶ dông: <color=0,255,0>%d<color><color>",
                                          timeinfo->tm_hour, timeinfo->tm_min, timeinfo->tm_sec, timeinfo->tm_mday,
                                          timeinfo->tm_mon + 1, timeinfo->tm_year + 1900,
                                          KSG_StringGetValue(3, pNode->nSaveEff, 1),
                                          KSG_StringGetValue(3, pNode->nSaveEff, 2),
                                          KSG_StringGetValue(3, pNode->nSaveEff, 3));
...
void KUiTongManager::OpenJiyuPopup(int nIndex) {
    ...
    for (int i = 0; i < nActionDataCount; i++) {
        if (nIndex == 0)
            strncpy(pSelUnitMenu->Items[pSelUnitMenu->nNumItem].szData, szArray_QingXiang[i],
                    sizeof(szArray_QingXiang[i]));
        else if (nIndex == 4)
