...
void KUiSkillTree::SaveConfig(KIniFile *pIni) {
    ...
    if (g_pCoreShell) {
        ...
        if (Skill.IMmediaSkill[0].uGenre != CGOG_NOTHING)
            pIni->WriteStruct(SKILLTREE_SHORTCUT_SKILL_SAVE, "LeftSkill", &Skill.IMmediaSkill[0],
                              sizeof(KUiGameObject));
        else
            pIni->EraseKey("Player", "LeftSkill");
        if (Skill.IMmediaSkill[1].uGenre != CGOG_NOTHING)
            pIni->WriteStruct(SKILLTREE_SHORTCUT_SKILL_SAVE, "RightSkill", &Skill.IMmediaSkill[1],
                              sizeof(KUiGameObject));
        else
            ...
            if (m_bLeft == (unsigned) ms_ShortcutSkills[j].IS_LEFT_SKILL &&
                ...
                if (pszKey) {
                    g_pRepresentShell->OutputText(m_nFont, pszKey, KRF_ZERO_END,
                                                  nLeft + m_nWidthPerSkill - m_nFont,
                                                  nTop + m_nHeightPerSkill - m_nFont - 1, m_uColor);
                    break;
                }
            }
        }
