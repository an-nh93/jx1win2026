...
void KUiEnchase::Initialize() {
    ...
    if (Ini.Load(Buff)) {
        Ini.GetString("RuleInfo", "Compound", "", m_CompoundPad.m_szTip[CPA_FUSION],
                      sizeof(m_CompoundPad.m_szTip[CPA_FUSION]));
        Ini.GetString("RuleInfo", "UpCryolite", "", m_CompoundPad.m_szTip[CPA_CRYOLITE],
                      sizeof(m_CompoundPad.m_szTip[CPA_CRYOLITE]));
        Ini.GetString("RuleInfo", "UpPropMine", "", m_CompoundPad.m_szTip[CPA_PROPMINE],
                      sizeof(m_CompoundPad.m_szTip[CPA_PROPMINE]));
...
#define    SCHEME_INI_UPGRADE        "∑÷“≥Õº∆◊.ini" // 2
    ...
    if (pItem) {
        ...
        for (int i = 0; i < MAX_COMPOUND_ITEM; i++) {
            if (CtrlItemMap[i].nPosition == pItem->Region.v) {
                if (bAdd)
                    m_UpgradeBox[i].HoldObject(pItem->Obj.uGenre, pItem->Obj.uId, pItem->Region.Width,
                                               pItem->Region.Height);
                else
...
#define    SCHEME_INI_COMPOUND        "∑÷“≥∫œ≥….ini" // 3 // chon khung ep huyen tinh
...
#define    SCHEME_INI_DISTILL        "∑÷“≥Ã·»°.ini" // 4
...
void KUiEnchase_Distill::UpdateItemObjBox(KUiObjAtRegion *pItem, int bAdd) {
    if (pItem) {
        ...
        for (int i = 0; i < MAX_COMPOUND_ITEM; i++) {
            if (CtrlItemMap[i].nPosition == pItem->Region.v) {
                if (bAdd)
                    m_DistillBox[i].HoldObject(pItem->Obj.uGenre, pItem->Obj.uId, pItem->Region.Width,
                                               pItem->Region.Height);
                else
...
#define    SCHEME_INI_FORGE        "∑÷“≥¥Ú‘Ï.ini" // 5
...
#define    SCHEME_INI_ENCHASE        "∑÷“≥œ‚«∂.ini" // 6
...
void KUiEnchase_Enchase::UpdateItemObjBox(KUiObjAtRegion *pItem, int bAdd) {
    if (pItem) {
        ...
        for (int i = 0; i < MAX_COMPOUND_ITEM; i++) {
            if (CtrlItemMap[i].nPosition == pItem->Region.v) {
                if (bAdd)
                    m_EnchaseBox[i].HoldObject(pItem->Obj.uGenre, pItem->Obj.uId, pItem->Region.Width,
                                               pItem->Region.Height);
                else
