...
#define        NO_SETTINGS                    "Kh?ng thi?t l?p"
...
void KUiRecovery::Initialize() {
    ...
    AddChild(&m_OtherSellTrashCK);      //Son auto ban rac
    AddChild(&m_OtherSellTrashTxt);
...
void KUiRecovery::LoadScheme(KIniFile *pIni) {
    ...
    m_OtherSellTrashCK.Init(pIni, "OtherSellTrashCK");  //Son auto ban rac
    m_OtherSellTrashTxt.Init(pIni, "OtherSellTrashTxt");
...
//	m_ChatNhamButton.Init(pIni, "ChatNhamButton");
    ...
    m_ChatNhamInputText = ":0Ch? ??:0, (xh) Auto (xh) V? L?m (!xh) Thi?n ?i?u (!xh).";
...
int KUiRecovery::WndProc(unsigned int uMsg, unsigned int uParam, int nParam) {
    switch (uMsg) {
        case WND_N_BUTTON_CLICK:
            ...
            //Fix
            else if (uParam == (unsigned int)(KWndWindow*)&m_OtherSellTrashCK) {
                if (m_OtherSellTrashCK.IsButtonChecked()) {
                    m_OtherSellTrashCK.CheckButton(true);
                    g_pCoreShell->PAIOperation(GPI_OTHER_SELL_TRASH, TRUE, NULL, NULL);
                }
                else
                {
                    m_OtherSellTrashCK.CheckButton(false);
                    g_pCoreShell->PAIOperation(GPI_OTHER_SELL_TRASH, FALSE, NULL, NULL);
                }
            }
            //end
            break;
...
void KUiRecovery::LoadSetting(KIniFile *pFile) {
    if (pFile) {
        ...
        m_OtherSellTrashCK.CheckButton(nValue > 0);
        g_pCoreShell->PAIOperation(GPI_OTHER_SELL_TRASH, nValue > 0, NULL, NULL);
...
void KUiRecovery::SaveSetting(KIniFile *pFile) {
    if (pFile) {
        ...
        pFile->WriteInteger(SAVE_SECTION, "SellTrash", m_OtherSellTrashCK.IsButtonChecked());
...
// "N?i l?c(?)", 		"manamax_v",
// "Th? l?c(?)", 		"staminamax_v",
// "Ph?c h?i SL(?)", 	"lifereplenish_v",
// "Ph?c h?i NL(?)", 	"manareplenish_v",
// "Ph?c h?i TL(?)", 	"staminareplenish_v",
// "H?t sinh l?c(%)", 	"steallifeenhance_p",
// "H?t n?i l?c(%)", 	"stealmanaenhance_p",
// "H?t th? l?c(%)", 	"stealstaminaenhance_p",
// "S?c m?nh(?)", 		"strength_v",
// "Th?n ph?p(?)", 	"dexterity_v",
// "Sinh kh?(?)", 		"vitality_v",
// "N?i c?ng(?)", 		"energy_v",
// "Ph?ng th?(%)", 	"physicsres_p",
// "Kh?ng ??c(%)", 	"poisonres_p",
// "Kh?ng b?ng(%)", 	"coldres_p",
// "Kh?ng h?a(%)", 	"fireres_p",
// "Kh?ng l?i(%)", 	"lightingres_p",
// "Kh?ng t?t c?(%)", 	"allres_p",
// "T?c ?? ch?y(%)", 	"fastwalkrun_p",
// "T?c ?? ??nh(%)", 	"attackspeed_v",
// "Ph?n ??n c?n(?)", 	"meleedamagereturn_v",
// "STVL ngo?i(?)", 	"addphysicsdamage_v",
// "B?ng s?t ngo?i(?)", "addcolddamage_v",
// "??c s?t ngo?i(?)", "addpoisondamage_v",
// "STVL ngo?i(%)", 	"addphysicsdamage_p",
// "CHSTTNL(%)", 		"damage2addmana_p",
// "May m?n(%)", 		"lucky_v",
// "B? qua n? tr?nh(%)", "ignoredefense_p",
// "?? ch?nh x?c(?)", 	"attackrating_v",
// "STLV n?i(?)", 		"addphysicsmagic_v",
// "B?ng s?t n?i(?)", 	"addcoldmagic_v",
// "H?a s?t n?i(?)", 	"addfiremagic_v",
// "L?i s?t n?i(?)", 	"addlightingmagic_v",
// "??c s?t n?i(?)", 	"addpoisonmagic_v",
// "Gi?m ch?m(%)", 	"freezetimereduce_p",
// "Gi?m tr?ng ??c(%)", "poisontimereduce_p",
// "Gi?m cho?ng(%)", 	"stuntimereduce_p",
// "Gi?m th??ng(%)", 	"fasthitrecover_v",
// "K? n?ng v?n c?(?)", "allskill_v",
// };
...
char szArray_Equipmagic[MAX_AUTO_FILTERL][2][64] =
        {
                ...
                "N?i l?c(?)", "manamax_v",
                "Th? l?c(?)", "staminamax_v",
                "Ph?c h?i sinh l?c(?)", "lifereplenish_v",
                "Ph?c h?i n?i l?c(?)", "manareplenish_v",
                "Ph?c h?i th? l?c(?)", "staminareplenish_v",
                "H?t sinh l?c(%)", "steallifeenhance_p",
                "H?t n?i l?c(%)", "stealmanaenhance_p",
                // "H?t th? l?c(%)", 	"stealstaminaenhance_p",
                "S?c m?nh(?)", "strength_v",
                ...
                "T?c ?? ch?y(%)", "fastwalkrun_p",
                "T?c ?? ??nh(%)", "attackspeed_v",
                "Ph?n ??n c?n chi?n(?)", "meleedamagereturn_v",
                "STVL ngo?i c?ng(?)", "addphysicsdamage_v",
                "B?ng s?t ngo?i c?ng(?)", "addcolddamage_v",
                "??c s?t ngo?i c?ng(?)", "addpoisondamage_v",
                "STVL ngo?i c?ng(%)", "addphysicsdamage_p",
                "Chuy?n h?a s?t th??ng th?nh n?i l?c(%)", "damage2addmana_p",
                "May m?n(%)", "lucky_v",
                "B? qua n? tr?nh(%)", "ignoredefense_p",
                "?? ch?nh x?c(?)", "attackrating_v",
                "STLV n?i c?ng(?)", "addphysicsmagic_v",
                "B?ng s?t n?i c?ng(?)", "addcoldmagic_v",
                "H?a s?t n?i c?ng(?)", "addfiremagic_v",
                "L?i s?t n?i c?ng(?)", "addlightingmagic_v",
                "??c s?t n?i c?ng(?)", "addpoisonmagic_v",
                "Th?i gian l?m ch?m(%)", "freezetimereduce_p",
                "Th?i gian tr?ng ??c(%)", "poisontimereduce_p",
                "Th?i gian cho?ng(%)", "stuntimereduce_p",
                "Th?i gian ph?c h?i(%)", "fasthitrecover_v",
                "K? n?ng v?n c?(?)", "allskill_v",
        };
...
char szArray_Team[][64] =
        {
                NO_SETTINGS,
                "T? nh?n t?t c? l?i m?i nh?m",
                "Ch? nh?n nh?m c? trong danh s?ch",
                "T? m?i nh?m t?t c?",
                "T? m?i nh?m theo danh s?ch",
                "T? ch?i v?o nh?m",
        };
...
void KUiOther::PlayerList(int uParam) {
    ...
    if (m_pNearbyPlayersList) {
        ...
        if (uParam == MENU_PLAYER) {
            for (int i = 0; i < MAX_AUTO_LIST; i++) {
                if (m_szPartyList[i][0]) {
                    strncpy(pSelUnitMenu->Items[pSelUnitMenu->nNumItem].szData, m_szPartyList[i],
                            sizeof(m_szPartyList[i]));
                    pSelUnitMenu->Items[pSelUnitMenu->nNumItem].szData[
                            sizeof(pSelUnitMenu->Items[pSelUnitMenu->nNumItem].szData) - 1] = 0;
