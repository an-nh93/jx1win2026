...
#define MARKET_DISCOUNT_PIC                    "\\spr\\Ui3\\????\\??????????\\%d_vn.spr"
...
void KUiShoppingCart::UpdateData() {
    if (g_pCoreShell) {
        ...
        m_GoodsInfo_ItemBox.HoldObject(m_ItemInfo.Obj.uGenre, m_ItemInfo.Obj.uId, m_ItemInfo.Region.Width,
                                       m_ItemInfo.Region.Height);
...
void KWndSellItem::Clear() {
    m_MarketGoods_DisCount.Hide();
    m_MarketGoods_imgNewArrival.Hide();
    m_GoodsNameText.Clear();//?????
    m_OriginalPriceText.Clear();//???
    m_OriginalPrice_NumberText.Clear();//???
    m_PriceText.Clear();//???
    m_Price_NumberText.Clear();//???
    m_ItemBox.Celar();
    m_MarketGoods_Buy.Enable(false);
}
...
int KWndSellItem::WndProc(unsigned int uMsg, unsigned int uParam, int nParam)//???????
{
    if (uMsg == WND_N_BUTTON_CLICK) {
...
void KUiSuperShop::CloseWindow(bool bDestroy) {
    if (m_pSelf) {
        ...
        //{
        //	UIMessageBox("Gi? h?ng ch?a thanh to?n. B?n mu?n h?y kh?ng?", m_pSelf, "X?c nh?n", "H?y b?", UI_WAIT_CLEAR_SHOPPINGCART);
        //}
        //else
        //{
...
void KUiSuperShop::SetSellTypeStart(int nStart) {
    ...
    for (i = m_nSellTypeStart; i < m_nStoreActive + m_nSellTypeStart; i++) {
        ...
        if (cTabFile.Load(BUYSELL_FILE)) {
            cTabFile.GetString((m_pBSinfo.m_nShopIdx[i] + 2), "?????", "", szName, sizeof(szName));
        } else {
    ...
    if (pItem) {
        if (pItem->Obj.uGenre != CGOG_MONEY) {
            ...
            if (g_pCoreShell->GetGameData(GDI_TRADE_ITEM_PRICE,
                ...
                m_WndSellItem[nAdd].m_OriginalPriceText.SetText("Gi? g?c:");
                m_WndSellItem[nAdd].m_PriceText.SetText("Gi? th?c:");
    ...
    if (bDoImmed == false) {
        ...
        if (eStatus == UIS_S_TRADE_BUY) {
            ...
            if ((GetKeyState(VK_SHIFT) & 0x8000) != 0) {
                ...
                strcpy(Msg.szMessage, "Nh?n gi? ph?m Shift ??ng th?i nh?n chu?t ph?i l?p t?c mua ???c v?t ph?m!");
