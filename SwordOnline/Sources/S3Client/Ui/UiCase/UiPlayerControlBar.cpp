...
void KUiPlayerControlBar::LoadScheme(const char *pScheme) {
    if (m_pSelf) {
        ...
        if (m_pSelf->Ini.Load(Buff)) {
            ...
            int nDiff, nStart, nBuffTop, ntxtBuffTop, i;
        ...
        if (m_BuffListCount) {
            ...
            if (nCount) {
                if (m_pStateSkill = (KStateControl *) malloc(sizeof(KStateControl) * nCount)) {
                    g_pCoreShell->GetGameData(GDI_NPC_STATE_SKILL, (unsigned int) m_pStateSkill,
                                              nCount);//?????????nCount?????
                    int nSkillID;
...
void KUiPlayerControlBar::Add(int nSkillPos, KStateControl *pStateSkill) {
    ...
    if (pStateSkill->nLeftTime <= GAME_FPS) {
        ...
        sprintf(szTemp, "%02dh:%02dm:%02ds", pStateSkill->nLeftTime / 18 / 60 / 60,
                pStateSkill->nLeftTime / 18 / 60 % 60, pStateSkill->nLeftTime / 18 % 60);
        strcat(pszDesc, szTemp);
