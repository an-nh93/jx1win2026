...
void KUiTrade::OnOppositeChangedItem(KUiObjAtRegion *pObj, int bAdd) {
    ...
    if (pObj->Obj.uGenre == CGOG_MONEY) {
        ...
        if (bAdd) {
            ...
            SetSpaceOverObjectDesc(this, 0, obj.uGenre, obj.uId, UOC_TO_BE_TRADE, x + ((obj.DataX + 1) * 26),
                                   y + ((obj.DataY) * 26), 3000, true);
        } else {
...
void KUiTrade::UpdateOperData(BOOL bSelfAsk, BOOL bDestReply) {
    ...
    if (bTrading) {
        ...
        m_Info.SetText(
                bOtherLock ? (bOtherTrading ? (m_bTradeFolkGame ? STR_OTHER_FOLKING : STR_OTHER_TRADING) : STR_OTHER_OK)
                           : STR_OTHER_NOT_OK, -1);
