...
void KUiSysMsgCentre::Initialize() {
    ...
    if (pSetting) {
        ...
        pSetting->GetInteger("Main", "SysMsgDisappearInterval", MSG_DISAPPEAR_INTERVAL_DEF,
                             (int *) (&m_uDisappearInterval));
...
void KUiSysMsgCentre::OnConfirmOperFinished(unsigned int uParam, int nSelAction) {
    ...
    switch (uParam) {
        case SMCT_UI_RENASCENCE:    //Ñ¡ÔñÖØÉú
            g_pCoreShell->OperationRequest(GOI_PLAYER_RENASCENCE, 0,
                                           nSelAction);//nSelAction=0£ºÔ­µØÖØÉú, nSelAction=1£º»Ø³Ç
            break;
        case SMCT_UI_TEAM_INVITE:    //´ðÓ¦»ò¾Ü¾ø¼ÓÈë¶ÓÎéµÄÑûÇë
            if (m_pHandlingMsg) {
                ...
                g_pCoreShell->TeamOperation(TEAM_OI_INVITE_RESPONSE,
                                            ...
                                            (nSelAction == 0));    //nSelAction=0£º´ðÓ¦, nSelAction=1£º¾Ü¾ø
            }
            break;
