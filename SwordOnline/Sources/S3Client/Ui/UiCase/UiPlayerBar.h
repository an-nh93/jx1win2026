...
public:
    ...
    static BOOL
    IsCanSendMessage(const char *Buffer, int nLen, char *szDestName, int nChannelID);                //能发送吗?
    static void
    OnSendChannelMessage(DWORD nChannelID, const char *Buffer, int nLen);                //发送频道聊天到服务器
    static void
    OnSendSomeoneMessage(const char *Name, const char *Buffer, int nLen);                //发送someone聊天到服务器
...
private:
    ...
    void OnDirectSendChannelMessage(DWORD nChannelID, BYTE cost, const char *Buffer,
                                    int nLen);                //发送频道聊天到服务器
    ...
    int IsHasCost(BYTE cost, int nMoney, int nLevel, int nMana, int nFullMana, int &nUseMoney, int &nUseLevel,
                  int &nUseMana);
