...
#define        DEFAULT_TASK                "\\spr\\ui3\\????????\\???????\\icon_task_003.spr"
...
void KUiTaskNote::LoadScheme(const char *pScheme) {
    if (m_pSelf) {
        ...
        if (Ini.Load(Buff)) {
            ...
            int i;
            for (i = 0; i < MAX_TASK_HEAD; i++) {
                sprintf(Buff, "%d", i);
                Ini.GetString("Main", Buff, DEFAULT_TASK, m_pSelf->m_szTaskImage[i][0],
                              sizeof(m_pSelf->m_szTaskImage[i][0]));
                sprintf(Buff, "%d_Tip", i);
                Ini.GetString("Main", Buff, NORMAL_UNCLEAR_WORD, m_pSelf->m_szTaskImage[i][1],
                              sizeof(m_pSelf->m_szTaskImage[i][1]));
            }
        ...
        if (nCnt > 0) {
            ...
            for (i = 0; i < nCnt; i++) {
                ...
                if (pRecord->bIsHeader) {
                    if (pRecord->uHeaderID >= (nSel * MAX_TASK_ON_PAGE) &&
                        pRecord->uHeaderID < ((nSel + 1) * MAX_TASK_ON_PAGE)) {
                        ...
                        m_TBtnTaskSel[j].btn.SetToolTipInfo(m_szTaskImage[pRecord->uHeaderID][1],
                                                            strlen(m_szTaskImage[pRecord->uHeaderID][1]));
                        m_TBtnTaskSel[j].id = pRecord->uHeaderID;
                        m_TBtnTaskSel[j].btn.SetPosition(nLeft + j * m_nDirLeft,
                                                         nTop + (j >= MAX_TASK_ON_PAGE ? m_nDirTop : 0));
                        m_TBtnTaskSel[j].onimg.SetPosition(nLeft + j * m_nDirLeft,
                                                           nTop + (j >= MAX_TASK_ON_PAGE ? m_nDirTop : 0));
                        m_TBtnTaskSel[j].bdimg.SetPosition(nLeft + j * m_nDirLeft,
                                                           nTop + (j >= MAX_TASK_ON_PAGE ? m_nDirTop : 0));
                        j++;
                    }
                }
...
void KUiTaskNote::UpdateList(int nId) {
    ...
    if (nId >= 0) {
        ...
        int nOffset = 0, nCnt = 0, nCntList = 0, nCntLine = 0, nListCount = KTaskDataFile::GetSystemHeaderCount(
                m_uCurHeaderId), i;
        nCnt = KTaskDataFile::GetSystemRecordCount();
        m_RecordList.GetMessageListBox()->SetCapability(nListCount);    //+4?????????
        for (i = 0; i < nCnt; i++) {
            const TASK_SYSTEM_RECORD *pRecord = KTaskDataFile::GetSystemRecord(i);
            if (pRecord == NULL)
                break;
