            ...
            if (uParam == (unsigned int) (KWndWindow *) &m_RoomList) {
                if (nParam >= 0 && m_bIsInRoom == FALSE) {
                    if (KSG_StringGetValue(1, m_RoomList.GetMessageListBox()->GetOneMessageData(nParam), 6) == 1)
                        ...
                        int nValue = 0;
                        nValue = KSG_StringSetValue(4, nValue, 1, KSG_StringGetValue(4,
                                                                                     m_RoomList.GetMessageListBox()->GetOneMessageData(
                                                                                             nParam), 1));
                ...
                if (nParam >= 0) {
                    if (GetKeyState(VK_CONTROL) & 0x8000) {
                        int nValue = 0;
                        nValue = KSG_StringSetValue(4, nValue, 1,
                                                    m_MemberList.GetMessageListBox()->GetOneMessageData(nParam));
                        nValue = KSG_StringSetValue(4, nValue, 2, 0);
                ...
                if (m_nRoomFigure == CHATROOM_HOST) {
                    if (GetKeyState(VK_CONTROL) & 0x8000) {
                        int nValue = 0;
                        nValue = KSG_StringSetValue(4, nValue, 1,
                                                    m_BlackList.GetMessageListBox()->GetOneMessageData(nParam));
...
void KUiChatRoom::ServerRequestBlackList(KOneMem *pMem) {
    if (m_pSelf) {
        m_pSelf->m_BlackList.GetMessageListBox()->AddOneMessage(pMem->lpszName, strlen(pMem->lpszName), 0,
                                                                pMem->nMemIndex);
    }
}
