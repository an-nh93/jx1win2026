...
*功能：增加一条消息在队列中
**********************************************************************************/
bool KUiNewsMessage2::AddMessage(KNewsMessage *pMsg, unsigned int uTime, unsigned int uTimeParam) {
...
void KUiNewsMessage2::PaintWindow() {
    if (m_pHandling && g_pRepresentShell) {
        ...
        if (m_bJustIncoming) {    //消息正在进入,(头)还未开始离开显示区域
...
bool KUiNewsMessage2::PickAMessage() {
    ...
    if (m_pHead == NULL) {
        if (ms_bEnableIdleMsg && uCurrentTime > m_uLastShowTime +
                                                m_uMaxIdleTime) {    //如果过了时间了，还没有新消息到，就随机在ini中选一条消息，插入队列中
            PickFromIni();
            if (m_pHandling = m_pHead)
