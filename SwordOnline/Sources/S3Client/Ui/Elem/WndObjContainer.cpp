...
void KWndObjectBox::PaintWindow() {
    ...
    if (m_Object.uGenre != CGOG_NOTHING && g_pRepresentShell && g_pCoreShell) {
        ...
        if (m_Style & OBJCONT_F_MOUSE_HOVER && (m_Style & OBJCONT_F_DISABLE_MOUSE_HOVER) == 0)
            ...
            if (Obj.Obj.uGenre == CGOG_ITEM || Obj.Obj.uGenre == CGOG_NPCSELLITEM ||
                Obj.Obj.uGenre == CGOG_PLAYERSELLITEM || Obj.Obj.uGenre == CGOG_PLAYERVIEWITEM) {
        ...
        Obj.nContainer = m_nContainerId;
        if (m_Object.uGenre == CGOG_ITEM || m_Object.uGenre == CGOG_NPCSELLITEM ||
            m_Object.uGenre == CGOG_PLAYERSELLITEM || m_Object.uGenre == CGOG_PLAYERVIEWITEM) {
            if (g_pCoreShell->GetGenreItem(m_Object.uId, Obj.Obj.uGenre) == item_equip)
                DrawBorder(m_Object.m_lastframe, m_Object.m_ucdelay, m_Object.m_posvs, m_Object.m_dirvs,
                           m_Object.m_vs_lastframe,
                           m_nAbsoluteLeft, m_nAbsoluteTop, m_Width, m_Height,
                           g_pCoreShell->GetNatureItem(m_Object.uId, Obj.Obj.uGenre));
        }
    }
}
    ...
    switch (uMsg) {
        ...
        case WM_MOUSEMOVE:
            ...
            if (m_Object.uGenre != CGOG_NOTHING && g_MouseOver.IsMoseHoverWndObj(this, 0) == 0 &&
                (m_Style & OBJCONT_F_DISABLE_MOUSE_HOVER) == 0) {
                SetMouseHoverObjectDesc(this, 0, m_Object.uGenre,
    ...
    for (int i = 0; i < m_nNumObjects; i++) {
        ...
        if (i == REPLACE_ITEM_INDEX(m_nPutPosX))
            ...
            if (Obj.nContainer == UOC_GAMESPACE)
                ...
                if (g_pCoreShell->GetGenreItem(Obj.Obj.uId, Obj.Obj.uGenre) == item_equip) {
                    if (eProp == IIEP_NORMAL && (pObj->uGenre == CGOG_ITEM || pObj->uGenre == CGOG_NPCSELLITEM ||
                                                 pObj->uGenre == CGOG_PLAYERSELLITEM) &&
                        g_UiBase.GetStatus() != UIS_S_TRADE_SETPRICE && !g_pCoreShell->GetTradeState())
                        Shadow.Color.Color_dw = l_BgColors[0];
                    else if (eProp == IIEP_NOT_USEABLE && g_UiBase.GetStatus() != UIS_S_TRADE_SETPRICE &&
                             !g_pCoreShell->GetTradeState())
                        Shadow.Color.Color_dw = l_BgColors[1];
                    else if (eProp == IIEP_SPECIAL && g_UiBase.GetStatus() != UIS_S_TRADE_SETPRICE &&
                             !g_pCoreShell->GetTradeState())
                        Shadow.Color.Color_dw = l_BgColors[2];
                }
                if (g_pCoreShell->GetTradePrice(Obj.Obj.uId) && g_UiBase.GetStatus() == UIS_S_TRADE_SETPRICE ||
                    g_pCoreShell->GetTradePrice(Obj.Obj.uId) && g_pCoreShell->GetTradeState()) {
        ...
        if (pObj->uGenre == CGOG_ITEM || pObj->uGenre == CGOG_NPCSELLITEM || pObj->uGenre == CGOG_PLAYERSELLITEM) {
            if (g_pCoreShell->GetGenreItem(pObj->uId, pObj->uGenre) == item_equip) {
                ...
                DrawBorder(pObj->m_lastframe, pObj->m_ucdelay, pObj->m_posvs, pObj->m_dirvs, pObj->m_vs_lastframe,
                           Shadow.oPosition.nX, Shadow.oPosition.nY, width, height,
                           g_pCoreShell->GetNatureItem(pObj->uId, pObj->uGenre));
            }
        }
    }
    if (m_nPutPosX >= 0) {
...
int KWndObjectMatrix::AddObject(KUiDraggedObject *pObject, int nCount) {
    if (pObject && nCount > 0) {
        ...
        if (nValidCount) {
            KUiDraggedObject *pNewList = (KUiDraggedObject *) realloc(m_pObjects, sizeof(KUiDraggedObject) *
                                                                                  (m_nNumObjects + nValidCount));
            if (pNewList) {
                m_pObjects = pNewList;
                for (i = 0; i < nCount; i++) {
                    if (pObject[i].uGenre) {
