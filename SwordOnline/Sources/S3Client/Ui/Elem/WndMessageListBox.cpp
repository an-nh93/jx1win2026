...
// 功能	: 新添一条信息
// -------------------------------------------------------------------------
int KWndMessageListBox::AddOneMessage(const char *pText, int nLen, unsigned int uTextBKColor, int nData/* = -1*/,
                                      KItemBtn *pIB/* = NULL*/) {
    if (pText == NULL)
        return 0;
    if (nLen < 0)
...
void KWndMessageListBox::SetSize(int nWidth, int nHeight) {
    ...
    if (nLines != m_nNumMaxShowLine || m_nNumBytesPerLine != nNumBytesPerLine) {
        ...
        if (m_Style & MSGLISTBOX_S_TEXTBOTTOM) {
            nTo = GetItemLineCount(m_nNumMessage) - GetItemLineCount(m_nStartShowMsg) - m_nStartMsgSkipLine -
                  m_nNumMaxShowLine;
            nTo = max(nTo, 0);
        }
        ...
        for (int i = 0; i < m_nNumMessage; i++) {
            if (g_pRepresentShell) {
                m_pMessages[i]->nLines = TGetEncodedTextLineCount(
                        m_pMessages[i]->Msg, m_pMessages[i]->nLen, m_nNumBytesPerLine, m_pMessages[i]->nCharWidth,
                        m_nFontSize, 0, 0, true);
            } else
...
void KWndMessageListBox::PaintWindow() {
    ...
    if (m_uTextLineShadowColor) {
        ...
        bg.oPosition.nY = y;
        bg.oEndPos.nY =
                bg.oPosition.nY + nNumVisibleTextLine * (m_nFontSize + 1 /*+ m_nRowDis*/); //TamLTM fix post item;
    ...
    while (nViewLines > 0 && nCurMsg < m_nNumMessage) {
        ...
        if (bDraw) {
            if (nCurMsg == m_nSelMsgIndex && (m_Style & MSGLISTBOX_S_SELABLE)) {
                ...
                if (m_SelMsgBgColor) {
                    ...
                    bg.oEndPos.nY = bg.oPosition.nY +
                                    Param.nNumLine * (m_nFontSize + 1  /* + m_nRowDis*/); //TamLTM fix post item;
                    g_pRepresentShell->DrawPrimitives(1, &bg, RU_T_SHADOW, true);
                }
            ...
            if (pCurMsg->uTextBKColor)    //绘制文字衬底
            {
                ...
                bg.oPosition.nY = Param.nY;
                bg.oEndPos.nX = bg.oPosition.nX + m_Width;
                bg.oEndPos.nY = bg.oPosition.nY + Param.nNumLine * (pCurMsg->nCharWidth * m_nFontSize /
                                                                    2);/*(m_nFontSize + 1 + m_nRowDis);*/ //TamLTM fix post item;
                g_pRepresentShell->DrawPrimitives(1, &bg, RU_T_SHADOW, true);
            }

            Param.bPicPackInSingleLine = true;
            //g_pRepresentShell->OutputRichText(m_nFontSize, &Param, pCurMsg->Msg, pCurMsg->nLen, m_Width);
            g_pRepresentShell->OutputRichText(m_nFontSize, &Param, pCurMsg->Msg + nOffset, pCurMsg->nLen - nOffset,
                                              m_Width); //TamLTM fix post item;

            if (m_bItemActived) {
                if ((m_pMessages[nCurMsg]->Item.nLine) > Param.nSkipLine && bPaintItem)
                    KUiPlayerControlBar::SetItemBtnPos(nCurMsg,
                                                       Param.nX + (m_pMessages[nCurMsg]->Item.nLeft) * m_nFontSize / 2 +
                                                       (m_pMessages[nCurMsg]->Item.nPic) * 2,
                                                       Param.nY +
                                                       ((m_pMessages[nCurMsg]->Item.nLine) - 1 - Param.nSkipLine) *
                                                       m_nFontSize);
                else
