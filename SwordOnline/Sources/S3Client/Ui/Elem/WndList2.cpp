...
void KWndList2::OnLButtonDown(int x, int y) {
    ...
    int nSel = m_nTopItemIndex - GetVisibleNodeCount(m_nTopItemIndex) +
               (y - m_nAbsoluteTop) / (m_nFontSize + 1 + m_nRowDis);
    if (nSel >= nCurNum)
        nSel = -1;
    ...
    if (nSel > -1) {
        ...
        if (ms_nDisableBtnTip == false && pItem->Tip[0] &&
            !g_MouseOver.IsMoseHoverWndObj((void *) (KWndWindow *) this, 0)) {
            char szTip[256];
...
void KWndList2::OnMouseMove(int x, int y) {
    int nCurNum = GetCurCount();
    int nIndex = m_nTopItemIndex - GetVisibleNodeCount(m_nTopItemIndex) +
                 (y - m_nAbsoluteTop) / (m_nFontSize + 1 + m_nRowDis);
    if (nIndex >= nCurNum) {
...
void KWndList2::PaintWindow() {
    ...
    if (g_pRepresentShell) {
        ...
        for (int i = m_nTopItemIndex; i < EndIndex; i++) {
            ...
            g_pRepresentShell->OutputText(m_nFontSize, pShowString, nLen, nLeft, y, Color, 0,
                                          TEXT_IN_SINGLE_PLANE_COORD, BorderColor);
