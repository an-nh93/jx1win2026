...
//-----------------------------------------------------------------------
// Npc[0]???????????????ã???????NpcSet?????????µ?NPC??
KNpc	Npc[MAX_NPC];


KNpcTemplate	* g_pNpcTemplate[MAX_NPCSTYLE]; //0,0????
...
void KNpc::Init()
{
	...
	m_CurrentLife = 100;			// Npc????????
	m_CurrentLifeMax = 100;		// Npc????????????
	m_CurrentLifeReplenish = 0;	// Npc??????????????
	m_CurrentLifeReplenishPercent = 0;
	m_CurrentMana = 100;			// Npc????????
	m_CurrentManaMax = 100;		// Npc???????????
	m_CurrentManaReplenish = 0;	// Npc??????????????
	m_CurrentStamina = 100;		// Npc????????
	m_CurrentStaminaMax = 100;	// Npc???????????
	m_CurrentStaminaGain = 0;	// Npc??????????????
	m_CurrentStaminaLoss = 0;	// Npc?????????½????
	m_CurrentAttackRating = 100;	// Npc??????????
	m_CurrentDefend = 10;		// Npc????????
	m_CurrentWalkSpeed = 5;		// Npc??????????
	m_CurrentRunSpeed = 10;		// Npc??????????
	m_CurrentJumpSpeed = 12;	// Npc??????????
	m_CurrentJumpFrame = 40;	// Npc??????????
	m_CurrentAttackSpeed = 0;	// Npc???????????
	m_CurrentCastSpeed = 0;		// Npc??????????
	m_CurrentVisionRadius = 40;	// Npc??????????
	m_CurrentAttackRadius = 30;	// Npc???????????
	m_CurrentHitRecover = 0;	// Npc?????????????
	m_CurrentAddPhysicsDamage = 0;	// Npc???????????????????
	...
	m_LifeMax					= 100;		// Npc?????????
	m_LifeReplenish				= 0;		// Npc????????????
	m_ManaMax					= 100;		// Npc?????????
	m_ManaReplenish				= 0;		// Npc????????????
	...
	m_AttackSpeed				= 0;		// Npc????????
	m_CastSpeed					= 0;		// Npc????????
	m_VisionRadius				= 40;		// Npc????????
	m_DialogRadius				= 124;		// Npc???????
	m_HitRecover				= 12;		// Npc???????????
	m_nPeopleIdx				= 0;
...
// ??????????????????????????????
	...
	if (m_StunState.nTime > 0)
	{
		m_DataRes.SetSpecialSpr("\\spr\\skill\\????\\mag_spe_???.spr");
		nRet = TRUE;
	}
...
if (IsPlayer())
	{
		...
		if (m_SkillList.CanCast(m_ActiveSkillID, SubWorld[m_SubWorldIndex].m_dwCurrentTime))
		{
			...
			case 1:
				...
				{
		/*------------------------------------------------------------------------------------
		?????????????????????????????Skill.Cast????????????????????-1,??????Npc index
		??S2C??????????????????Server??NpcIndex??NpcdwID??????
		??C?????????????NpcdwID????????NpcIndex
			-------------------------------------------------------------------------------------*/
#ifdef _SERVER	// ?????9??Region????????
...
//	g_DebugLog("Npc[nLauncher].m_ActiveSkillID %d", Npc[nLauncher].m_ActiveSkillID);
	...
	//5 Damage doc sat
/*	int  totalDamagePoison1 = rand() % 2;
	int  totalDamagePoison2;

	if (nMissleSeries == 4) //0=kim - 1=moc - 2=thuy - 3=hoa - 4=tho
	{
		totalDamagePoison2 = 2;
	}
	else if (nMissleSeries == 3 || nMissleSeries == 4)
	{
		totalDamagePoison2 = 1;
	}
	else if (nMissleSeries == 0 || nMissleSeries == 1) // ky he kim
	{
		totalDamagePoison2 = rand() % 2 ;
	}
	else
	{
		totalDamagePoison2 = 0;
	}*/
	pTemp++;
	if (CalcDamage(nLauncher, nMissleSeries,
...
void KNpc::Load(int nNpcSettingIdx, int nLevel)
{
	...
	else
	{
		...
		if (!szNpcTypeName[0])
		{
			g_NpcKindFile.GetString(2, "CharacterName", "", szNpcTypeName, sizeof(szNpcTypeName));//???û??????õ????npc????
		}
...
void KNpc::PaintTop(int nHeightOffset, int nnHeightOffset, int nFontSize, DWORD dwBorderColor)
{
	...
	if (m_PTrade.nTrade)
	{
		...
		RUIconImageR.nISPosition = IMAGE_IS_POSITION_INIT;
		RUIconImageR.bRenderFlag = RUIMAGE_RENDER_FLAG_REF_SPOT;
		strcpy(RUIconImageR.szImage, "\\spr\\Ui3\\???\\????????????.spr");
...
{
	int len;
	fgets(m_nNamePK, 256, stdin);
	{ char* p = strchr(m_nNamePK, '\n'); if (p) *p = '\0'; }
...
{
	m_CurrentLucky = Player[m_nPlayerIdx].m_nLucky;	// ????????????????????????????????????????
}
...
void KNpc::GetNpcCopyFromTemplate(int nNpcTemplateId)
{
	...
	if (g_pNpcTemplate[nNpcTemplateId]) //?????????????????????????
...
int KNpc::ChangeWorld(DWORD dwSubWorldID, int nX, int nY)
{
	...
	if (IsPlayer())
		Player[m_nPlayerIdx].m_nPrePayMoney = 0;// ???????????????û??
	// ???????????????
