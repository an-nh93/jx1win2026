...
//	Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].CheckTimerMoveBarrier();
	...
	if (nMapX == nOldMapX && nMapY == nOldMapY && nRegion == nOldRegion)
		return Obstacle_NULL;
	return m_Region[nRegion].GetBarrierMin(nMapX, nMapY, nDx, nDy, TRUE, TRUE);

#else

	if (nMapX == nOldMapX && nMapY == nOldMapY && nRegion == nOldRegion)
	{
		return m_Region[nRegion].GetBarrierMin(nMapX, nMapY, nDx, nDy, FALSE, FALSE);
	}
...
BOOL KSubWorld::LoadMap(int nId, int nRegion)
{
	...
	if (!m_Region)
	{
        g_DebugLog("Son debug LOAD MAP 1");
		m_Region = new KRegion[MAX_REGION];
	}

	if (nId != m_SubWorldID)
	{
        g_DebugLog("Son debug LOAD MAP 2");
		...
		//g_ScenePlace.DirectFindPos(0, 0, FALSE, FALSE);	//绘画
	...
	if (nIdx < 0)
	{
        g_DebugLog("Son debug LOAD MAP 3:");
