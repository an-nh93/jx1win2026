...
void KMissle::OnFly()
{
    ...
    case	MISSLE_MMK_Line:							//	直线飞行 --TamLTM Fix danh 1 duong thang phia truoc - skill missle
    {
        //TamLTM Fix check toc do huong di cua vu khi hoang kim ngu doc giao = id: 355 Skill 9x huyen am tram //m_WeaponType == 23 la vu khi hoang kim
        //KSkill* pOrdinSkill = (KSkill*)g_SkillManager.GetSkill(m_nSkillId, m_nLevel);
        if (m_nSkillId == 355 && Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].m_WeaponType == 23)
        {
            m_nSpeed = 35;
            //	g_DebugLog("m_nSpeed: %d", m_nSpeed);
        }

        if (m_nSkillId == 355 && m_nSpeed > 32)
        {
            m_nSpeed = 35;
        }

        nDOffsetX = (m_nSpeed * m_nXFactor + 50);
        nDOffsetY = (m_nSpeed * m_nYFactor + 50);
        //	g_DebugLog("m_nYFactor: %d-%d", m_nSpeed, m_nYFactor);
            //end code
    }
    break;
...
BOOL	KMissle::CheckBeyondRegion(int nDOffsetX, int nDOffsetY)
{
    ...
    //TamLTM debug loi messle qua' muc tieu
    _ASSERT(abs(nNewXOffset) <= CellWidth * 10); //Cu~
    _ASSERT(abs(nNewYOffset) <= CellHeight * 10); //Cu~ TamLTM Fix Debug
