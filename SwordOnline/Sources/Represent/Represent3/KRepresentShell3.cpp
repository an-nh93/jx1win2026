...
#include "d3d_utils.h"
/* dxerr9.h removed - deprecated in modern SDK; use FormatMessage for D3D errors if needed */
...
inline unsigned int KRepresentShell3::GetPoint3dLighting(D3DXVECTOR3& v)
{
	...
	return pLightingArray[
		// ?????????uY * LIGHTING_GRID_WIDTH / LIGHTING_GRID_SIZEY,???uY / LIGHTING_GRID_SIZEY?????????????????????
...
IInlinePicEngineSink* g_pIInlinePicSinkRP = NULL;	//??????????????[wxb 2003-6-20]
HRESULT KRepresentShell3::AdviseRepresent(IInlinePicEngineSink* pSink)
{
	assert(NULL == g_pIInlinePicSinkRP);	//???????????
...
uint32 KRepresentShell3::BuildMesh3D(VERTEX3D *pvbSrc, VERTEX3D* pvbDes, uint32 nNumDes, DWORD color)
{
	...
	{
		VERTEX3D vt1 = pvbSrc[0];						// ???????????
		VERTEX3D vt2 = pvbSrc[0];						// ???????????
		float ftuAdd, ftvAdd;
		bool bAllDone = false;
