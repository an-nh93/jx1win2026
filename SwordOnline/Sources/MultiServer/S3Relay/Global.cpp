...
const DWORD _localmask = 0x0000FFFF;
const DWORD _localtag = 0x0000A8C0;
...
_DATABASEINFO m_DatabaseInfo; //TamLTM fix xu;
S3PDBConVBC* pConnectDB;//TamLTM fix xu;
...
BOOL gInitialize()
{
	...
	TCHAR m_szINIPath[MAX_PATH + 1];
	TCHAR m_szServer[def_KEYNAMELEN];
	TCHAR m_szDataBase[def_KEYNAMELEN];
	TCHAR m_szUser[def_KEYNAMELEN];
	TCHAR m_szPassword[def_KEYNAMELEN];
	KPIGetExePath(m_szExePath, MAX_PATH);
	_tcscpy(m_szINIPath, m_szExePath);
	_tcscat(m_szINIPath, "DataBase.ini");

	if (m_szINIPath)
	{
		KPIGetPrivateProfileString(def_SELECTKEYNAME, def_SERVERKEYNAME, "", m_szServer, def_KEYNAMELEN, m_szINIPath);
		KPIGetPrivateProfileString(def_SELECTKEYNAME, def_DATABASEKEYNAME, "", m_szDataBase, def_KEYNAMELEN, m_szINIPath);
		KPIGetPrivateProfileString(def_SELECTKEYNAME, def_USERKEYNAME, "", m_szUser, def_KEYNAMELEN, m_szINIPath);
		KPIGetPrivateProfileString(def_SELECTKEYNAME, def_PASSWORDKEYNAME, "", m_szPassword, def_KEYNAMELEN, m_szINIPath);

		m_DatabaseInfo.strServer = m_szServer;
