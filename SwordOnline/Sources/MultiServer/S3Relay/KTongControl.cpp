...
BOOL	CTongControl::Instate(STONG_INSTATE_COMMAND *pInstate, STONG_INSTATE_SYNC *pSync)
{
	...
	if (channid != -1)
	{
		...
		{
		case enumTONG_FIGURE_DIRECTOR:
			sprintf(szMsg, "%s ???c b? nhi?m l?m %s ", szName, defTONG_DIRECTOR_AGNAME);
			break;
		case enumTONG_FIGURE_MANAGER:
			if (pSync->m_btNewFigure > pSync->m_btOldFigure)
				sprintf(szMsg, "%s ???c b? nhi?m l?m %s ", szName, defTONG_MANAGER_AGNAME);
			else
				sprintf(szMsg, "%s b? b?i nhi?m tr? th?nh %s ", szName, defTONG_MANAGER_AGNAME);
			break;
		case enumTONG_FIGURE_MEMBER:
			sprintf(szMsg, "%s b? b?i nhi?m tr? th?nh %s ", szName, defTONG_MEMBER_AGNAME);
			break;
		}
...
BOOL	CTongControl::Kick(STONG_KICK_COMMAND *pKick, STONG_KICK_SYNC *pSync)
{
	...
	if (channid != -1)
	{
		sprintf(szMsg, "%s ?? b? tr?c xu?t kh?i b?n bang", szName);
		g_ChannelMgr.SayOnChannel(channid, TRUE, std::string(), std::string(MESSAGE_TONG_ANNOUCE_HEAD), std::string(szMsg));
	}
...
BOOL	CTongControl::Leave(STONG_LEAVE_COMMAND *pLeave, STONG_LEAVE_SYNC *pSync)
{
	...
	if (channid != -1)
	{
		sprintf(szMsg, "%s ch? ??ng r?i kh?i b?n bang", szName);
		g_ChannelMgr.SayOnChannel(channid, TRUE, std::string(), std::string(MESSAGE_TONG_ANNOUCE_HEAD), std::string(szMsg));
	}
...
BOOL	CTongControl::AcceptMaster(STONG_ACCEPT_MASTER_COMMAND *pAccept)
{
	...
	// TamTLM Nhuong chuc bang chu lai cho %s
	DWORD channid = g_ChannelMgr.GetChannelID(szMsg, 0);
	if (channid != -1)
	{
		sprintf(szMsg, "Bang ch? %s ?? nh??ng ch?c v? bang ch? l?i cho %s ", szOldMaster, szName);
		g_ChannelMgr.SayOnChannel(channid, TRUE, std::string(), std::string(MESSAGE_TONG_ANNOUCE_HEAD), std::string(szMsg));
	}
...
BOOL CTongControl::ChangeMoney(STONG_MONEY_COMMAND* pMoney, STONG_MONEY_SYNC *Sync)
{
	...
	case enumC2S_TONG_MONEY_SAVE:
		...
		if (channid != -1)
		{
			sprintf(szMsg, "%s ?? ??ng g?p %d ng?n l??ng v?o v?o tr??ng m?c ng?n kh? b?n bang", pMoney->m_szName,pMoney->m_dwMoney);	// gui tien vao bang
			g_ChannelMgr.SayOnChannel(channid, TRUE, std::string(), std::string(MESSAGE_TONG_ANNOUCE_HEAD), std::string(szMsg));
		}
		break;
	case enumC2S_TONG_MONEY_GET:
		...
		if (channid != -1)
		{
			sprintf(szMsg, "%s ?? r?t kh?i tr??ng m?c ng?n kh? %d ng?n l??ng", pMoney->m_szName,pMoney->m_dwMoney);	// lay tien khoi bang
			g_ChannelMgr.SayOnChannel(channid, TRUE, std::string(), std::string(MESSAGE_TONG_ANNOUCE_HEAD), std::string(szMsg));
		}
		break;
	}
...
BOOL CTongControl::DBChangeAgname(STONG_ACCEPT_AGNAME_COMMAND *pAccept)
{
	...
	if (pAccept->m_btFigure == enumTONG_FIGURE_DIRECTOR)
	{
		...
		sprintf(szMsg, "%s %s ???c thay ??i danh hi?u th?nh: %s ", defTONG_DIRECTOR_AGNAME, m_szDirectorName[i], szAgname);
	}
	else if (pAccept->m_btFigure == enumTONG_FIGURE_MANAGER)
	{
		for (i = 0; i < defTONG_MAX_MANAGER; i++)
		...
		sprintf(szMsg, "%s %s ???c thay ??i danh hi?u th?nh: %s ", defTONG_MANAGER_AGNAME, m_szManagerName[i], szAgname);
	}
	else
	{
		return FALSE;
...
BOOL CTongControl::DBChangeSexAgname(STONG_ACCEPT_SEX_AGNAME_COMMAND *pAccept)
{
	...
	if (pAccept->m_btSex)
	{
		...
		sprintf(szMsg, "Thay ??i danh hi?u n? ?? t? trong bang th?nh: %s", szAgname);
	}
	else
	{
		...
		sprintf(szMsg, "Thay ??i danh hi?u nam ?? t? trong bang th?nh: %s", szAgname);
	}
...
BOOL CTongControl::DBChangeCamp(STONG_CHANGE_CAMP_COMMAND *pInfo, STONG_BE_CHANGED_CAMP_SYNC *sInfo)
{
	...
	case camp_justice:
		...
		if (channid != -1)
		{
			sprintf(szMsg, "Phe ph?i hi?n t?i c?a b?n bang l?: Ch?nh ph?i");
			g_ChannelMgr.SayOnChannel(channid, TRUE, std::string(), std::string(MESSAGE_TONG_ANNOUCE_HEAD), std::string(szMsg));
		}
		break;
	case camp_evil:
		...
		if (channid != -1)
		{
			sprintf(szMsg, "Phe ph?i hi?n t?i c?a b?n bang l?: T? ph?i");
			g_ChannelMgr.SayOnChannel(channid, TRUE, std::string(), std::string(MESSAGE_TONG_ANNOUCE_HEAD), std::string(szMsg));
		}
		break;
	case camp_balance:
		...
		if (channid != -1)
		{
			sprintf(szMsg, "Phe ph?i hi?n t?i c?a b?n bang l?: Trung l?p");
			g_ChannelMgr.SayOnChannel(channid, TRUE, std::string(), std::string(MESSAGE_TONG_ANNOUCE_HEAD), std::string(szMsg));
		}
		break;
	}
...
BOOL CTongControl::DBChangeTongRecruit(STONG_CHANGE_TONG_INFO_COMMAND *pInfo, STONG_CHANGE_TONG_INFO_SYNC *sInfo)
{
	...
	if (m_bRecruit)
		sprintf(szMsg,"Bang ch? m? ch?c n?ng chi?u m? ?? t? ");
	else
		sprintf(szMsg,"Bang ch? ??ng ch?c n?ng chi?u m? ?? t? ");
	if (channid != -1)
		g_ChannelMgr.SayOnChannel(channid, TRUE, std::string(), std::string(MESSAGE_TONG_ANNOUCE_HEAD), std::string(szMsg));
	return TRUE;
}
...
BOOL CTongControl::DBChangeTongLevel(STONG_CHANGE_TONG_INFO_COMMAND *pInfo, STONG_CHANGE_TONG_INFO_SYNC *sInfo)
{
	...
	if (pInfo->m_nValue > 0)
	{
		...
		sprintf(szMsg, "\\O%u", m_dwNameID);
		DWORD channid = g_ChannelMgr.GetChannelID(szMsg, 0);		
		sprintf(szMsg,"??ng c?p bang h?i hi?n t?i l?: %d", m_btLevel);
		if (channid != -1)
			g_ChannelMgr.SayOnChannel(channid, TRUE, std::string(), std::string(MESSAGE_TONG_ANNOUCE_HEAD), std::string(szMsg));
		return TRUE;
	}
...
BOOL CTongControl::DBChangeTongMoney(STONG_CHANGE_TONG_INFO_COMMAND *pInfo, STONG_CHANGE_TONG_INFO_SYNC *sInfo)
{
	...
	if (pInfo->m_nValue > 0)
	{
		...
		sprintf(szMsg, "\\O%u", m_dwNameID);
		DWORD channid = g_ChannelMgr.GetChannelID(szMsg, 0);
		if (pInfo->m_MessStr == 1)
		{
			sprintf(szMsg, "<color=255,107,107>Ti?n thu thu? bang h?i chi?m l?nh hi?n t?i ???c %d l??ng<color>", pInfo->m_AddMoneyThue); //TamLTM thong bao tien thu thue bang hoi chiem linh
		}
		else
		{
			sprintf(szMsg, "Ng?n qu? bang h?i hi?n t?i l? %d l??ng", m_dwMoney); // Set ngan quy bang hoi hien tai
		}
...
BOOL CTongControl::DBChangeTongEff(STONG_CHANGE_TONG_INFO_COMMAND *pInfo, STONG_CHANGE_TONG_INFO_SYNC *sInfo)
{
	...
	if (pInfo->m_nValue > 0)
	{
		...
		sprintf(szMsg, "\\O%u", m_dwNameID);
		DWORD channid = g_ChannelMgr.GetChannelID(szMsg, 0);
		sprintf(szMsg, "C?ng hi?n t?n bang h?i hi?n t?i l? %d ?i?m", m_dwTotalEff);
		if (channid != -1)
			g_ChannelMgr.SayOnChannel(channid, TRUE, std::string(), std::string(MESSAGE_TONG_ANNOUCE_HEAD), std::string(szMsg));	
		return TRUE;
	}
...
BOOL CTongControl::DBChangeJiyu(STONG_CHANGE_TONG_INFO_COMMAND *pInfo, STONG_CHANGE_TONG_INFO_SYNC *sInfo)
{
	...
	if (channid != -1)
	{
		sprintf(szMsg,"Bang ch? ?? ti?n h?nh B?o l?u tin t?c bang h?i");
		g_ChannelMgr.SayOnChannel(channid, TRUE, std::string(), std::string(MESSAGE_TONG_ANNOUCE_HEAD), std::string(szMsg));
	}
	return TRUE;
}
		...
		case enumC2S_CHANGE_TONG_MEMBEREFF:
		{
			...
			switch(pInfo->m_btFigure)
			{
				...
				if (channid != -1)
				{
					if(pInfo->m_nValue2)
					{
					sprintf(szMsg, "%s ?? ??ng g?p %d ?i?m c?ng hi?n v?o tr??ng m?c c?ng hi?n b?n bang", pInfo->m_szName, pInfo->m_nValue2);
					g_ChannelMgr.SayOnChannel(channid, TRUE, std::string(), std::string(MESSAGE_TONG_ANNOUCE_HEAD), std::string(szMsg));
					}
					TMemberStruct	sMember;
