...
int	g_nTongPSSize[defTONG_PROTOCOL_SERVER_NUM] = 
{
	...
	sizeof(STONG_GET_EXTPOINT_COMMAND),			// enumC2S_GET_EXTPOINT //TamLTM fix xu;
...
void CTongConnect::Proc0_Tong(const void* pData, size_t size)
{
	...
	case enumC2S_MESSAGE_TO_CHATROOM:
		...
		break;
	case enumC2S_GET_EXTPOINT://TamLTM fix xu;
		{
			STONG_GET_EXTPOINT_COMMAND* pExtPoint = (STONG_GET_EXTPOINT_COMMAND*)pData;
			int nExtPoint = g_cTongSet.GetExtPoint(pExtPoint);
			STONG_GET_EXTPOINT_SYNC	nExt;
			nExt.ProtocolFamily = pf_tong;
			nExt.ProtocolID = enumS2C_SET_EXTPOINT;
			nExt.m_nExtPoint = nExtPoint;
			nExt.m_dwParam = pExtPoint->m_dwParam;
			this->SendPackage((const void*)&nExt, sizeof(nExt));
		}
	break;
		case enumC2S_UPDATE_EXTPOINT://TamLTM fix xu;
		{
			STONG_UPDATE_EXTPOINT_COMMAND* pExtPoint = (STONG_UPDATE_EXTPOINT_COMMAND*)pData;
			g_cTongSet.UpdateExtPoint(pExtPoint);
		}
		break;
	default:
		break;
