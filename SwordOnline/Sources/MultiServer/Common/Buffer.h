...
private:
	// Tag type to avoid C2956: operator delete(void*, size_t) conflicts with C++14+ sized deallocation
	struct buffer_placement_t { size_t value; explicit buffer_placement_t(size_t v) : value(v) {} };

	static void *operator new( size_t objSize, buffer_placement_t bp );
	static void operator delete( void *pObj );
	static void operator delete( void *pObj, buffer_placement_t bp );
