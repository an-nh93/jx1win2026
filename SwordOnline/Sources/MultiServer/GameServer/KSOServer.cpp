...
//------------------------ Connect tong end ----------------------------
	...
	m_pCoreServerShell->OperationRequest(SSOI_TONG,(unsigned int)pTongClient, 0);
	...
	case s2c_roleserver_saverole_result:
		{
			...
			//TamLTM Check code thay doi ten nhan vat
			if (pPD->szAccountName[0])
			{
		...
		case c2c_transferroleinfo:
			{
				...
				if ( pGI->datalength > 0 )
				{
					...
					if (bCRC)
					{
						nIdx = m_pCoreServerShell->AddCharacter(pGI->nExtPoint, pGI->nChangePoint, (void *)pRoleData, &guid);//TamLTM fix xu;
					}
					else
					{
...
//				SavePlayerData(nIndex);
				...
				sLeaveGame.nExtPoint = m_pCoreServerShell->GetGameData(SGDI_CHARACTER_EXTPOINTCHANGED, nIndex, 0);//TamLTM fix xu;
	...
	case c2c_notifyexchange:
		{
			...
			gi.nExtPoint = m_pCoreServerShell->GetGameData(SGDI_CHARACTER_EXTPOINT, nIndex, 0);//TamLTM fix xu;
			gi.nChangePoint = m_pCoreServerShell->GetGameData(SGDI_CHARACTER_EXTPOINTCHANGED, nIndex, 0);//TamLTM fix xu;
		...
		case s2c_syncgamesvr_roleinfo_cipher:
			{
				...
				if ( pGI->datalength > 0 )
				{
					...
					{
						int nIdx = m_pCoreServerShell->AddCharacter(pGI->nExtPoint, pGI->nChangePoint, (void *)pRoleData, &guid);//TamLTM fix xu;
...
//	{
		...
		case s2c_querymapinfo:
			{
				...
				pUMI = ( tagUpdateMapID * )new int[ nMapCount * sizeof(short) + sizeof( tagUpdateMapID ) ];
				memset( pUMI, 0, nMapCount * sizeof(short) + sizeof( tagUpdateMapID ) );

				pUMI->cProtocol = c2s_updatemapinfo;
				pUMI->cMapCount = nMapCount;
				memcpy( pUMI->szMapID, sMapID, nMapCount * sizeof(short));

				/*printf("-----Send \ndatalen = %u, data = ", nMapCount * sizeof(char) + sizeof( tagUpdateMapID ));
				for (int i = 0; i < 32; i++)
					printf("[%02X] ", ((BYTE *)pUMI)[i]);
				printf("\n");*/
				m_pGatewayClient->SendPackToServer( ( const void * )pUMI, nMapCount * sizeof(short) + sizeof( tagUpdateMapID ) );
		...
		case s2c_gateway_broadcast:
			...
			break;
		//case s2c_punish:
			//GatewayPunishProcess((const char*)pData, dataLength); //TamLTM check and fix kill it
		//	break;
		}
...
//#ifndef _STANDALONE
	...
	case AP_LOCK_ACCOUNTNAME:
			if(m_pCoreServerShell->OperationRequest(SSOI_LOCK_ACCOUNTNAME, (unsigned int)pGatewayBroadCast->szData, strlen(pGatewayBroadCast->szData)))
				printf("Lock Account: %s", pGatewayBroadCast->szData);
		break;
	case AP_UNLOCK_ACCOUNTNAME:
			if(m_pCoreServerShell->OperationRequest(SSOI_UNLOCK_ACCOUNTNAME, (unsigned int)pGatewayBroadCast->szData, strlen(pGatewayBroadCast->szData)))
				printf("Unlock Account: %s", pGatewayBroadCast->szData);
		break;
	case AP_LOCK_CHAT:
			if(m_pCoreServerShell->OperationRequest(SSOI_LOCK_CHAT, (unsigned int)pGatewayBroadCast->szData, strlen(pGatewayBroadCast->szData)))
				printf("Lock Speech: %s", pGatewayBroadCast->szData);
		break;
	case AP_UNLOCK_CHAT:
			if(m_pCoreServerShell->OperationRequest(SSOI_UNLOCK_CHAT, (unsigned int)pGatewayBroadCast->szData, strlen(pGatewayBroadCast->szData)))
...
//TamLTM Ham call send ping len client -> len server
...
BOOL KSwordOnLineSever::SavePlayerData(int nIndex, bool bUnLock)
{
	...
	memset(sProcessData.szAccountName, 0, sizeof(sProcessData.szAccountName)); //Doi ten nhan vat
	m_pCoreServerShell->GetLastName(nIndex, sProcessData.szAccountName); //Doi ten nhan vat
...
void KSwordOnLineSever::PlayerLogoutGateway()
{
	...
	{
		// Clear player which Connect gateway but not Connect gamesvr
