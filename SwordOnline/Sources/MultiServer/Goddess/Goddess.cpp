...
{
//	MessageBox( NULL, "TamLTM Test ThreadProcess was error!", "Warning", MB_OK );
...
int APIENTRY WinMain(HINSTANCE hInstance,
                     ...
                     int       nCmdShow)
{
	//TamLTM check code co them code o day
	...
	case WM_TIMER:
		{
			...
			//TamLTM Check suspend backup databse
			if(IsBackupThreadWorking())
			{
				...
				//	MessageBox( NULL, "TamLTM Test ThreadProcess was error!", "IsBackupWorking", MB_OK );
				}
				...
				//	MessageBox( NULL, "TamLTM Test ThreadProcess was error!", "g_IsBackupSuspend", MB_OK );
				}		
			}
			else
			{
...
//			::EnableWindow( GetDlgItem( hwnd, IDC_EDIT_BACKUP_BEGIN_TIME ), FALSE );
	...
	case WM_CREATE_ENGINE:
		...
		char szText[64];
		sprintf(szText, "%04d%02d%02d", timeinfo->tm_year+1900, timeinfo->tm_mon+1, timeinfo->tm_mday); // Time limit server
