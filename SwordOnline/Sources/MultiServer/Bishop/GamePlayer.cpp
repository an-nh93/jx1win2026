...
bool CGamePlayer::_UnlockAccount()
{
	...
	user.nExtPoint = 0; //TamLTM fix xu;
...
//	userlogin.MacNum = m_MACNum;	// anti 3 acc by veg
...
UINT CGamePlayer::VerifyAccount()
{
	...
	cprintf( "CGamePlayer::VerifyAccount Successful!\n" );
		...
		//	m_sAccountName = pReturn->Account;//TamLTm fix kill it;
	...
	cprintf( "CGamePlayer::VerifyAccount Failed! [Already login]\n" );
			...
			break;
		//TamLTM Fix code
	/*	case E_ACCOUNT_ENOUGH:		/* Login Failed ! --- Already login */

/*#ifdef	CONSOLE_DEBUG
	cprintf( "CGamePlayer::VerifyAccount Failed! [Account enough]\n" );
#endif

			nQueryResult |= LOGIN_R_ENOUGH;
...
/*	bool ok = false;
	...
	//TamLTM campare co sua if ngay cho nay account
	if(strcmp(m_sAccountName.c_str(), pRoleData->BaseInfo.caccname)==0)
	{
		...
		if ( pGServer )
		{
			...
			ok = pGServer->DispatchTask( CGameServer::enumSyncRoleInfo, pData, dataLength, max(m_nExtPoint, 0) ); //TamLTm fix xu;
			m_nExtPoint = -1;	//TamLTm fix xu;
		}
	}*/
	...
	//			( IPlayer * )this );
		...
		//fix
		if(CGamePlayer::Get( ( const char * )pRoleData->BaseInfo.szName) == NULL)
