...
bool CGameServer::_UpdateMapID( const void *pData, size_t datalength )
{
    ...
    short *pMapID = (short*)pUMI->szMapID; // TamLTM fix check thay doi BYTE *pMapID = pUMI->szMapID; cai nay BYTE -> int
    //int	*pMapID = (int *)pUMI->szMapID;
...
bool CGameServer::_NotifyLeaveGame( const void *pData, size_t datalength )
{
	...
	return PopAccount( ( const char * )pAccountName, ( pLG->cCmdType != HOLDACC_LEAVEGAME ), pLG->nExtPoint);//TamLTM fix xu;
}
...
bool CGameServer::FreezeMoney( const char *pAccountName, WORD nExtPoint )
{
	...
	user.Version = ACCOUNT_CURRENT_VERSION;
	user.nExtPoint = nExtPoint; //TamLTM fix xu;
...
bool CGameServer::_SyncRoleInfo( const void *pData, size_t datalength, WORD nData)
{
	...
	ginfo.nExtPoint = nData;//TamLTM fix xu;
	ginfo.nChangePoint = 0;//TamLTM fix xu;
