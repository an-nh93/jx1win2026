...
int S3PAccount::Login(S3PDBConVBC* pConn, const char* strAccName, const char* strPassword, DWORD ClientID, WORD& nExtPoint, DWORD& nLeftTime/*, DWORD MacNum*/)
{
	...
	//TamLTM code tai day fix
	const char pass[100]="')DELETE Account_Info --";
		if (strcmp(strAccName,pass)==0)
		return FALSE;

	const char name[100]="')DELETE Account_Habitus --";
		if (strcmp(strAccName,name)==0)
		return FALSE;
	//end code
			...
			if (clientID.vt == VT_NULL || clientID.lVal == 0)
			{
				...
				// {
					if (GetLeftSecondsOfDeposit(pConn, strAccName, iLeft, iExp) == ACTION_SUCCESS && iLeft > 1800)	//最小剩余时间30分钟
					{
						...
						//Set nMAC SQL TamLTM Fix again
						
						//limit login windown
/*						if (pResult->num_rows() > 4)	// anti 3 acc by veg
						{
							sprintf(strSQL, "update Account_info set iClientID = 0, nMac = 0 where (cAccName = '%s')", strAccName);
							return true;
						}*/
					//	sprintf(strSQL, "UPDATE Account_Info SET iClientID = '%d', nMac = '%d', dLoginDate = null where (cAccName = '%s')", ClientID, MacNum, strAccName);

						sprintf(strSQL, "update Account_info set iClientID = %d, dLoginDate = null where (cAccName = '%s')", ClientID, strAccName);//Fix
...
int S3PAccount::ElapseTime(S3PDBConVBC* pConn, DWORD ClientID, const char* strAccName, const DWORD dwDecSecond)
{
	...
	if (iRet == ACTION_SUCCESS)
	{
		...
		char strSQL[MAX_PATH];
		sprintf(strSQL, "update View_AccountMoney set iLeftSecond = iLeftSecond - %d where (cAccName = '%s') and (datediff(second, getdate(), dEndDate) <= 0)", dwSecord, strAccName);
		if (pConn->Do(strSQL))
		{
			iRet = ACTION_SUCCESS;
		}
										...
										long& liExp)
{
	...
	//sprintf(strSQL, "EXEC GetExPoint_Account '%s'", strAccName); // fix xu
