...
BOOL KGatewayDataProcess::ProAccountLogin(S3PDBConVBC* pConn, const IBYTE* lpData, const DWORD dwDataSize)
{
	...
	if (NULL != lpData && dwDataSize == sizeof(KAccountUserLoginInfo))
	{
			...
			)
		{
			...
			aReturn.nLeftTime = aReturn.nExtPoint = 0;//TamLTM fix xu
			...
			if (m_UserNames.find(*((UserName*)pAccInfo->Account)) == m_UserNames.end())	//不在黑名单上
			{
				if (pAccInfo->Type == AccountUserLoginInfo)
				{
					...
					aReturn.nReturn = S3PAccount::Login(pConn, pAccInfo->Account, pAccInfo->Password, m_nGameID, aReturn.nExtPoint, aReturn.nLeftTime/*, pAccInfo->MacNum*/);	// anti 3 acc by veg
				}
				else
			...
			{
				bRet = TRUE;
				sprintf(suc, " Success ! (Ext:%d)", aReturn.nExtPoint);//TamLTM fix xu;
				szDetail = suc;
...
/*				else if (aReturn.nReturn == E_ACCOUNT_ENOUGH) //anti 3 acc by veg
					szDetail = " Failed ! --- Account Enough";*/
				else
					szDetail = " Failed !";
			}
			if (pAccInfo->Type == AccountUserLoginInfo)
...
BOOL KGatewayDataProcess::ProAccountLogout(S3PDBConVBC* pConn, const IBYTE* lpData, const DWORD dwDataSize)
{
	...
	if (NULL != lpData && dwDataSize == sizeof(KAccountUserLogout))
	{
			...
			pAccInfo->Type == AccountUserLogout)
		{
			...
			aReturn.nReturn = S3PAccount::Logout(pConn, m_nGameID, pAccInfo->Account, pAccInfo->nExtPoint); //TamLTM fix xu;
			...
			{
				bRet = TRUE;
				sprintf(suc, " Success ! (Ext:%d)", pAccInfo->nExtPoint);//TamLTM fix xu;
				szDetail = suc;
			}
			else
			{
